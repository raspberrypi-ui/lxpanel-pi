...
void lxplug_write_settings (config_setting_t *settings, conf_table_t *conf_table)
{
    ...
    while (cptr->type != CONF_TYPE_NONE)
    {
        switch (cptr->type)
        {
            case CONF_TYPE_BOOL :
            case CONF_TYPE_INT :
                config_group_set_int (settings, cptr->name, (int) ((long) *cptr->value));
                break;
            ...
            case CONF_TYPE_COLOUR :
                char *str = gdk_rgba_to_string ((GdkRGBA *) cptr->value);
                config_group_set_string (settings, cptr->name, str);
                g_free (str);
                break;
